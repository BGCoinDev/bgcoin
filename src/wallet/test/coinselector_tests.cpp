...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(result14->GetSelectedValue(), 1 * COIN);  // we should get 1 BGCO in 1 coin
        ...
        BOOST_CHECK_EQUAL(result15->GetSelectedValue(), 2 * COIN);  // we should get 2 BGCO in 1 coin
    ...
    // test randomness
    {
        ...
        for (int i = 0; i < RUN_TESTS; i++) {
            ...
            const auto result25 = KnapsackSolver(GroupCoins(available_coins.All()), 4 * COIN, CENT);
            BOOST_CHECK(result25);
            const auto result26 = KnapsackSolver(GroupCoins(available_coins.All()), 4 * COIN, CENT);
...
BOOST_AUTO_TEST_CASE(coin_grinder_tests)
{
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 60; ++j) { // 60 UTXO --> 19,8 BGCO total --> 60 × 272 WU = 16320 WU
                add_coin(available_coins, wallet, CAmount(0.33 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            for (int i = 0; i < 10; i++) { // 10 UTXO --> 20 BGCO total --> 10 × 272 WU = 2720 WU
                add_coin(available_coins, wallet, CAmount(2 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            return available_coins;
        });
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            // Expected Result: 4 + 3 + 2 + 1 = 10 BGCO at 400 vB
...
BOOST_AUTO_TEST_CASE(srd_tests)
{
...

    {
        ...
        const auto& res = SelectCoinsSRD(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 10; ++j) {
                /* 10 × 1 BGCO + 10 × 2 BGCO = 30 BGCO. 20 × 272 WU = 5440 WU */
...
BOOST_AUTO_TEST_CASE(check_max_selection_weight)
{
    ...
    {
        // Scenario 1:
        // The actor starts with 1x 4.0 BGCO and 1515x 0.033 BGCO (~100.0 BGCO total) unspent outputs
        // Then tries to spend 49.5 BGCO
        // The 4.0 BGCO output should be selected, because the transaction would otherwise be too large
        ...
        const auto result = select_coins(
            target, cs_params, cc, [&](CWallet& wallet) {
                ...
                add_coin(available_coins, wallet, CAmount(4 * COIN), CFeeRate(0), 144, false, 0, true);
                return available_coins;
            },
            m_node);

        BOOST_CHECK(result);
        // Verify that the 50 BGCO UTXO was selected, and result is below max_weight
        BOOST_CHECK(has_coin(result->GetInputSet(), CAmount(4 * COIN)));
...

    {
        ...
        // The actor starts with 400x 0.0625 BGCO and 2000x 0.025 BGCO (75.0 BGCO total) unspent outputs
        // Then tries to spend 49.5 BGCO
...

    {
        ...
        // The actor starts with 1515x 0.033 BGCO (49.995 BGCO total) unspent outputs
...
BOOST_AUTO_TEST_CASE(SelectCoins_effective_value_test)
{
    ...
    CoinsResult available_coins;
    {
        ...
        add_coin(available_coins, *dummyWallet, 100000); // 0.001 BGCO
    }

    CAmount target{99900}; // 0.000999 BGCO
