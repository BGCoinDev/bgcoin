...
- ClientModel is used by the main application `bgcoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a bgcoin:... link from a browser).
...
* `bgcoinstrings.cpp`: automatically generated
* `bgcoinunits.(h/cpp)`: BGCO / mBGCO / etc. handling
* `callback.h`
...
**Note:** Do not change `local/bgcoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `bgcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bgcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `bgcoin-qt`
