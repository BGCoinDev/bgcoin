...
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // in later releases by https://github.com/BGCoinDev/bgcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bgcoin 22.x will not try to read these.
...
//! Convert enabled/size values to bgcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from bgcoin -prune setting.
...
//! Get pruning size value to show in GUI from bgcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    if (!settings.contains("DisplayBitcoinUnit")) {
        settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(BitcoinUnit::BGCO));
    }
    QVariant unit = settings.value("DisplayBitcoinUnit");
    if (unit.canConvert<BitcoinUnit>()) {
        m_display_bgcoin_unit = unit.value<BitcoinUnit>();
    } else {
        m_display_bgcoin_unit = BitcoinUnit::BGCO;
        settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(m_display_bgcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bgcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bgcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitcoinUnit>() == m_display_bgcoin_unit) return;
    m_display_bgcoin_unit = new_unit.value<BitcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(m_display_bgcoin_unit));
    Q_EMIT displayUnitChanged(m_display_bgcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/BGCoinDev/bgcoin/pull/8273
    ...
    // (https://github.com/bgcoin-core/gui/issues/567).
