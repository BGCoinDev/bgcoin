...
static bool ParseArgs(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/bgcoin.conf are parsed in qt/bgcoin.cpp's main()
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bgcoind -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  bgcoind [options]                     Start " PACKAGE_NAME "\n"
                "\n";
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for bgcoind but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect bgcoind signal handlers
    noui_connect();
