...
BOOST_AUTO_TEST_CASE(package_cpfp_tests)
{
    ...
    const CAmount coinbase_value{4 * COIN};
    ...
    // Now that the child's fees have "increased" by 1 BGCO, the cheap package should succeed.
    {
    ...
    // Parent pays 1 BGCO and child pays none. The parent should be accepted without the child.
    {
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
