...
FUZZ_TARGET(tx_package_eval, .init = initialize_tx_pool)
{
    ...
    for (const auto& outpoint : g_outpoints_coinbase_init_mature) {
        ...
        outpoints_value[outpoint] = 4 * COIN;
    }
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 300)
    {
        ...
        if (fuzzed_data_provider.ConsumeBool()) {
            ...
            const auto delta = fuzzed_data_provider.ConsumeIntegralInRange<CAmount>(-4 * COIN, +4 * COIN);
        ...
        if (fuzzed_data_provider.ConsumeBool()) {
            client_maxfeerate = CFeeRate(fuzzed_data_provider.ConsumeIntegralInRange<CAmount>(-1, 4 * COIN), 100);
        }
