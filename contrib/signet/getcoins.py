...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bgcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bgcoin-cli', help='bgcoin-cli command to use')
...
parser.add_argument('bgcoin_cli_args', nargs='*', help='Arguments to pass on to bgcoin-cli (default: -signet)')

args = parser.parse_args()

if args.bgcoin_cli_args == []:
    args.bgcoin_cli_args = ['-signet']


def bgcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bgcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bgcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bgcoin_cli(['getnewaddress', 'faucet', 'bech32'])
