...
def main(bgcoind_path):
    bgcoind_with_usdts = USDT(path=str(bgcoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    bgcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    bgcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    bgcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    bgcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[bgcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/bgcoind")
        exit(1)
