...
- The oldest supported Python version is specified in [doc/dependencies.md](/doc/dependencies.md).
  ...
  The CI linter job also checks this, but [possibly not in all cases](https://github.com/BGCoinDev/bgcoin/pull/14884#discussion_r239585126).
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x50=1250 BGCO) in its wallet. Using them is much more
...
with the bgcoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a bgcoind.  `P2PInterface`
...
Taken from the [python-bgcoinrpc repository](https://github.com/jgarzik/python-bgcoinrpc).
...
Test objects for interacting with a bgcoind node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-bgcoinlib)
