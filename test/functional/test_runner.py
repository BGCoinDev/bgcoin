...
BASE_SCRIPTS = [
    ...
    'interface_bgcoin_cli.py --legacy-wallet',
    'interface_bgcoin_cli.py --descriptors',
...
def main():
    ...
    enable_bgcoind = config["components"].getboolean("ENABLE_BITCOIND")

    if not enable_bgcoind:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if bgcoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "bgcoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a bgcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    # a hard link or a copy on any platform. See https://github.com/BGCoinDev/bgcoin/pull/27561.
...
class RPCCoverage():
    ...
    commands per `bgcoin-cli help` (`rpc_interface.txt`).
