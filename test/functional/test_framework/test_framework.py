...
TMPDIR_PREFIX = "bgcoin_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    """Base class for a bgcoin test script.

    Individual bgcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave bgcoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop bgcoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bgcoin-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "bgcoind": ("bgcoind", "BITCOIND"),
            "bgcoin-cli": ("bgcoincli", "BITCOINCLI"),
            "bgcoin-util": ("bgcoinutil", "BITCOINUTIL"),
            "bgcoin-wallet": ("bgcoinwallet", "BITCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: bgcoinds were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bgcoind', self.options.bgcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bgcoin-cli', self.options.bgcoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bgcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple bgcoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the bgcoind
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bgcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    bgcoind=self.options.bgcoind,
                    bgcoin_cli=self.options.bgcoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bgcoin.conf
    ...
    def skip_if_no_bgcoind_tracepoints(self):
        """Skip the running test if bgcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bgcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bgcoind_zmq(self):
        """Skip the running test if bgcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bgcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bgcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bgcoin-wallet has not been compiled")

    def skip_if_no_bgcoin_util(self):
        """Skip the running test if bgcoin-util has not been compiled."""
        if not self.is_bgcoin_util_compiled():
            raise SkipTest("bgcoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bgcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bgcoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether bgcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bgcoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bgcoin_util_compiled(self):
        """Checks whether bgcoin-util was compiled."""
