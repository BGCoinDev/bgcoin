...
redundancy and robustness for both the BGCoin network and individual nodes.
...
CJDNS peers than Tor or IP ones. You can use `bgcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/BGCoinDev/bgcoin/blob/master/doc/tor.md) for
details.
...
To see which CJDNS peers your node is connected to, use `bgcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bgcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bgcoin-cli help getnodeaddresses` for details.
