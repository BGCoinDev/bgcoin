...
The headless daemon `bgcoind` has the JSON-RPC API enabled by default, the GUI
`bgcoin-qt` has it disabled by default. This can be changed with the `-server`
...
This is the endpoint used by bgcoin-cli when a `-rpcwallet=` parameter is passed in.
...
bgcoin-cli createwallet mywallet false false "" false false true

# "params": {"wallet_name": "mywallet", "load_on_startup": true}
bgcoin-cli -named createwallet wallet_name=mywallet load_on_startup=true

# "params": {"args": ["mywallet"], "load_on_startup": true}
bgcoin-cli -named createwallet mywallet load_on_startup=true
```
...
- **Securing remote network access:** You may optionally allow other
  ...
  `bgcoind -help` for more information about these settings and other
